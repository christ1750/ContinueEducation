package edu.neu.nxist.continuingedu.excel;
...
public class ExcelUtil {
	...
	public static <T> void listToExcel(List<T> list,
			...
			OutputStream out, Project project) throws ExcelException {

		listToExcel(list, fieldMap, sheetName, 65535, out, project);
	...
	public static <T> void listToExcel(List<T> list,
			LinkedHashMap<String, String> fieldMap, String sheetName,
			int sheetSize, HttpServletResponse response, Project project)
			throws ExcelException {
		...
		try {
			...
			listToExcel(list, fieldMap, sheetName, sheetSize, out, project);
	...
	public static <T> void listToExcel(List<T> list,
		...
		listToExcel(list, fieldMap, sheetName, 65535, response, project);
	}
	...
	public static <T> void listToExcel(List<T> list,
			LinkedHashMap<String, String> fieldMap, String sheetName,
			int sheetSize, OutputStream out, Project project)
			throws ExcelException {
	...
	private static <T> void fillSheet(WritableSheet sheet, List<T> list,
			...
			int lastIndex, Project project) throws Exception {

		/* 表头 */
		// 设置字体的大小、颜色等样式
		WritableFont wf1 = new WritableFont(WritableFont.createFont("宋体"), 20,
		...
		wcf1.setWrap(true);//自动换行

		/* 二级表头 */
		// 设置字体的大小、颜色等样式
		WritableFont wf2 = new WritableFont(WritableFont.createFont("宋体"), 12,
				WritableFont.BOLD, false);
		...
		/* 主体样式 */
		// 设置字体的大小、颜色等样式
		WritableFont wf4 = new WritableFont(WritableFont.createFont("宋体"), 10,
		...
		Label label0 = new Label(0, 0, project.getProjectName() + "报名情况统计表",
				wcf1);
		...
		Label label1 = new Label(0, 2, "项目类别："
				+ project.getProjectCategory().getPjCategoryName(), wcf2);
		Label label2 = new Label(2, 2, "项目编号：" + project.getProjectOrder(),
				wcf2);
		Label label3 = new Label(4, 2, "项目名称：" + project.getProjectName(), wcf2);
		...
		for (int i = 0; i <= cnFields.length; i++) {
			Label label;
			if (i == 0)
				label = new Label(i, 3, "序号", wcf3);
			else
				label = new Label(i, 3, cnFields[i-1], wcf3);
			sheet.addCell(label);
		}
		...
		for (int index = firstIndex; index <= lastIndex; index++) {
			...
			for (int i = 0; i <= enFields.length; i++) {
				if (i == 0) {
					jxl.write.Number label = new jxl.write.Number(i, rowNo, rowNo-3,
							wcf4);
					sheet.addCell(label);
				} else {
					Object objValue = getFieldValueByNameSequence(enFields[i-1],
							item);
					String fieldValue = objValue == null ? "" : objValue
							.toString();
					Label label = new Label(i, rowNo, fieldValue, wcf4);
					sheet.addCell(label);
				}
			}
