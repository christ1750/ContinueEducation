							...
							<s:iterator value="#request.guestbooks.list" var="x">
								<tr onmouseover="$(this).addClassName('mouse_over');"
									...
									<td>
										...
										</div> <br /> <span> <strong><s:property
													value="#x.uwUsername" /> </strong > <s:property value="#x.CTime" />
									</span><span><a onclick="respond(<%=index%>)" style="cursor: pointer;">回复</a> <input
					...
					<form action="<%=path%>/guestbookAction!addGuestbook"
						...
						<div>
							<textarea class="required max-length-150" cols="80" id="comment"
								name="CComment" rows="7" style="resize: none;"></textarea>
						</div>
						<INPUT type="submit" value="提交" onclick="affirm()"
							style="width:55px;height:24px;cursor:pointer;margin-top: 3px;" />
